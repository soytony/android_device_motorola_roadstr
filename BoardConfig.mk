...
# Images supplied by the device tree. Keep the stock firmware prerequisites in
# the same custom-image plumbing as the replacement boot logo so they are
# carried into target-files and release image packages. These are installer
# prerequisites, not Android partitions produced by the build, so do not add
# them to flash-all.
